<script lang="ts">
	...
	const countOfUpcomingTasks: number = $derived(
		filterTasks($tasks, TaskFilterType.PENDING, undefined).length
	);
</script>
...
<div class="dock static md:hidden">
	<button
		...
		onclick={async () => await goto('/app/dashboard')}
	>
		<HomeIcon />
	...
	<button
		class:dock-active={page.url.pathname.startsWith('/app/tasks')}
		onclick={async () => await goto('/app/tasks')}
	>
		<ClipboardCheckIcon />
	...
	<button
		class:dock-active={page.url.pathname === '/app/expenses'}
		onclick={async () => await goto('/app/expenses')}
	>
		<CashIcon />
	...
	<button
		class:dock-active={page.url.pathname === '/app/statistics'}
		onclick={async () => await goto('/app/statistics')}
	>
		<StatsReportIcon />
	...
	<button
		class:dock-active={page.url.pathname.startsWith('/app/settings')}
		onclick={async () => await goto('/app/settings')}
	>
		<SettingsIcon />
		<span class="dock-label">{m['menu.settings']()}</span>
	</button>
</div>
<div class="flex w-full justify-around bg-base-200 max-sm:hidden">
	<ul class="menu menu-horizontal rounded-box bg-base-200">
		...
		<li>
			<a class:menu-active={page.url.pathname.startsWith('/app/tasks')} href="/app/tasks">
				<ClipboardCheckIcon />
				{m['menu.tasks']()}
				<span class="badge badge-xs" class:hidden={countOfUpcomingTasks === 0}
					>{countOfUpcomingTasks}</span
				>
			</a>
		</li>
		<li>
