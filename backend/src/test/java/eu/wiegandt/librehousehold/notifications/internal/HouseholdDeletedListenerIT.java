...
/**
 * Publishes {@link HouseholdDeleted} through the real {@code ApplicationEventPublisher} (unlike
 * {@code AccountRegistrationListenerIT}/{@code PasswordResetRequestedListenerIT}, which call the
 * listener directly): the event already carries all member/household data, so no household/member
 * rows need to exist in the database beforehand, and publishing for real additionally proves that
...
class HouseholdDeletedListenerIT {
    ...
    @Test
    void householdDeleted_threeMembers_sendsThreeEmails() {
        // given
        var householdName = "Musterhaushalt";
        var members = List.of(
                new HouseholdDeleted.DeletedMember(UUID.randomUUID(), "Anna", "anna@example.com"),
                new HouseholdDeleted.DeletedMember(UUID.randomUUID(), "Bea", "bea@example.com"),
                new HouseholdDeleted.DeletedMember(UUID.randomUUID(), "Carl", "carl@example.com"));
        doReturn(new MimeMessage((Session) null)).when(mailSender).createMimeMessage();

        // when
        new TransactionTemplate(transactionManager).executeWithoutResult(status ->
                eventPublisher.publishEvent(new HouseholdDeleted(UUID.randomUUID(), householdName, members)));

        // then
        await().atMost(Duration.ofSeconds(5))
                .untilAsserted(() -> verify(mailSender, times(3)).send(any(MimeMessage.class)));
    }
}
