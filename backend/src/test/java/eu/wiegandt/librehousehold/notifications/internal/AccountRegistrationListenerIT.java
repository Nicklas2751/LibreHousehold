...
/**
 * Calls the listener method directly instead of publishing {@link AccountRegistered} through the
 * real {@code ApplicationEventPublisher}: {@code @ApplicationModuleListener} still runs
 * {@code @Async} even when invoked directly on the autowired (proxied) bean, so the assertions
 * below poll with Awaitility rather than asserting immediately after the call returns.
 */
...
class AccountRegistrationListenerIT {
    ...
    @Test
    void accountRegistered_issuesVerificationTokenAndSendsEmail() {
        // given
        ...
        // when
        listener.on(new AccountRegistered(member.getId(), member.email()));

        // then
        await().atMost(Duration.ofSeconds(5)).untilAsserted(() -> {
            assertThat(accountTokenRepository.findAll())
                    .anyMatch(token -> token.memberId().equals(member.getId()) && token.purpose().equals("EMAIL_VERIFICATION"));
