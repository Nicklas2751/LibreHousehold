...
class ExpenseServiceIT {
    ...
    class getDebtorExpenses {
        ...
        @Test
        void expenseInConfirmedSettlement_isExcluded() {
            // given
            var householdId = UUID.randomUUID();
            var payerId = UUID.randomUUID();
            var debtorId = UUID.randomUUID();
            ...
            var confirmedSettlement = new ReimbursementEntity(UUID.randomUUID(), householdId,
                    BigDecimal.ONE, payerId, debtorId, "CONFIRMED", null, LocalDateTime.now());
            confirmedSettlement.setCoveredExpenses(Set.of(new SettlementExpenseRef(expense.getId())));
        ...
        @Test
        void expenseNotInConfirmedSettlement_isIncluded() {
            // given
            var householdId = UUID.randomUUID();
            var payerId = UUID.randomUUID();
            var debtorId = UUID.randomUUID();
            ...
            // settlement with empty coveredExpenses — does not cover the expense
            var confirmedSettlement = new ReimbursementEntity(UUID.randomUUID(), householdId,
                    BigDecimal.ONE, payerId, debtorId, "CONFIRMED", null, LocalDateTime.now());
            reimbursementRepository.save(confirmedSettlement);
        ...
        void pendingSettlementIsNotCutoff_expenseIsIncluded() {
            ...
            var pendingSettlement = new ReimbursementEntity(UUID.randomUUID(), householdId,
                    BigDecimal.ONE, payerId, debtorId, "PENDING", null, LocalDateTime.now());
            pendingSettlement.setCoveredExpenses(Set.of(new SettlementExpenseRef(expense.getId())));
            reimbursementRepository.save(pendingSettlement);

            // when
            var result = expenseService.getDebtorExpenses(householdId, payerId, debtorId);

            // then: PENDING settlements don't exclude from debtor expenses
            assertThat(result).extracting(Expense::getId).containsExactly(expense.getId());
        }
    }
    ...
    class getExpenses {
        ...
        @Test
        void expenseInCoveredExpenses_isMutableFalse() {
            // given
            var householdId = UUID.randomUUID();
            ...
            settlement.setCoveredExpenses(Set.of(new SettlementExpenseRef(expense.getId())));
            ...
            assertThat(result).singleElement()
                    .satisfies(e -> assertThat(e.getIsMutable()).contains(false));
        }

        @Test
        void expenseNotInCoveredExpenses_isMutableTrue() {
            // given
            var householdId = UUID.randomUUID();
            var creditorId = UUID.randomUUID();
            var debtorId = UUID.randomUUID();
            ...
            // settlement with empty coveredExpenses — does not cover the expense above
            var settlement = new ReimbursementEntity(UUID.randomUUID(), householdId,
                    BigDecimal.ONE, creditorId, debtorId, "CONFIRMED", null, LocalDateTime.now());
            reimbursementRepository.save(settlement);
            ...
            // then
            assertThat(result).singleElement()
                    .satisfies(e -> assertThat(e.getIsMutable()).contains(true));
        }
    }
}
