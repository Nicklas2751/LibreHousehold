...
The decision was deferred as an open question until authentication needed to be implemented.
...
- The setup wizard and registration flow have an unambiguous rule: an e-mail with an existing account can never start a second household — this must be rejected with a clear, actionable error instead of a generic 409.
